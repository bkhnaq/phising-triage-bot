...
_OBSERVABLE_GROUPS = {
    "url": "urls", "domain": "domains", "ip": "ips", "email": "emails",
    "md5": "hashes", "sha1": "hashes", "sha256": "hashes", "filename": "filenames",
}
...
def suggested_playbook(risk: dict, language: dict | None = None) -> str:
    ...
    return "suspicious_email" if risk.get("base_score", risk.get("score", 0)) else "email_review"
...
def build_siem_event(
    ...
    observables: dict[str, list[str]] = {name: [] for name in ("urls", "domains", "ips", "emails", "hashes", "filenames")}
    observable_metadata = {}
    records = sorted(result.get("observables", []), key=lambda item: (item["type"], item["value"]))
    ...
    for index, finding in enumerate(
        ...
        finding_details[f"f{index:03d}"] = {
            key: finding[key] for key in (
                "type", "summary", "category", "severity", "confidence", "status",
                "evidence", "evidence_groups", "source_findings", "consumed_evidence",
                "score_contribution",
            ) if key in finding
        }
    mismatch_types = sorted({
        str(item.get("type", "")) for item in header.get("findings", [])
        if "mismatch" in str(item.get("type", "")) or "alignment" in str(item.get("type", ""))
    })
    if any("does not match relay server" in warning for warning in relay.get("warnings", [])):
        mismatch_types.append("relay_mismatch")
    keywords = sorted({
        str(item["keyword"]) for item in result.get("heuristics", {}).get("suspicious_keywords", [])
    } | {
        keyword for item in result.get("url_intelligence", {}).get("suspicious_endpoints", [])
        for keyword in item.get("keywords", [])
    })
    return _clean({
        "schema_version": "1.0",
        "integration": "phishing_bot", "event_type": "email_triage",
        "event_id": identifier,
        "timestamp": timestamp or prior.get("timestamp") or datetime.now(timezone.utc).isoformat(),
        "email": {key: str(metadata.get(key) or "") for key in (
            "subject", "from", "to", "date", "message_id", "return_path", "reply_to"
        )},
        "authentication": {
            check: AuthState.parse(auth.get(check, {}).get("state") or auth.get(check, {}).get("result")).value
            for check in ("spf", "dkim", "dmarc")
        },
        "header_analysis": {
            **{key: str(header.get(key) or "") for key in (
                "from_domain", "return_path_domain", "reply_to_domain", "message_id_domain"
            )},
            "origin_ip": str(relay.get("origin_ip") or ""),
            "relay_domains": sorted({str(hop["server"]) for hop in relay.get("relay_chain", []) if hop.get("server")}),
        ...
        "url_analysis": {
            ...
            "deceptive_link_details": {
                f"l{index:03d}": {key: item.get(key, "") for key in (
                    "url", "displayed_url", "displayed_domain", "actual_domain", "state",
                    "deceptive_link", "requires_redirect_validation"
        ...
        "content_analysis": {
            ...
            "credential_request": bool({"credential_harvesting", "account_verification", "password_expiration"} & categories.keys()) or bool(result.get("credential_harvesting", {}).get("detected")),
        ...
        "ai": {
            ...
            "status": "UNAVAILABLE" if ai.get("error") or ai.get("verdict", "unknown") == "unknown" else "ANALYZED",
        },
        ...
        "risk": {
            ...
            "initial_severity": risk.get("initial_severity", risk.get("risk_severity", "LOW")),
            "initial_verdict": risk.get("initial_verdict", risk.get("verdict", "BENIGN")),
        },
        "evidence_coverage": {
