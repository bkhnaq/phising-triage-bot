...
def test_pipeline_emits_one_event_per_email_without_network(no_network, tmp_path):
    ...
    pipeline = PhishingPipeline(
        upload_dir=str(tmp_path / "uploads"), events_jsonl_path=str(log)
    )
    ...
    for result, event in zip((first, second), events):
        ...
        assert {
            ...
            "multi_signal_credential_phishing",
        } <= set(event["correlated_findings"])
        assert all(
            item["environment"] == "TEST" and item["exportable"] is False
            for item in event["observable_metadata"].values()
        )
        assert "final_score" not in serialize_event(result)
        assert (
            not {"whois", "dns", "virustotal", "otx", "redirect"}
            & event["evidence_coverage"]["sources"].keys()
        )
...
def test_public_shorteners_and_public_origin_never_trigger_enrichment(
    no_network, tmp_path
):
...
@pytest.mark.parametrize(
    "displayed",
    [
        "https://safe.test/account",
        "https://evil.test/account",
        "https://sub.evil.test/login",
    ],
)
def test_displayed_destination_mismatch_includes_path_and_subdomain(displayed):
    ...
    assert (
        next(item for item in urls if item["url"] == "https://evil.test/login")[
            "deceptive_link"
        ]
        is True
    )
...
def test_ioc_export_preserves_url_semantics_and_ipv6():
    ...
    records = collect_observables(
        urls=extract_urls(url), attachments=[], url_intelligence=None
    )
    ...
    assert any(
        item["type"] == "ip" and item["value"] == "2001:db8::7" for item in records
    )


def test_observables_include_headers_body_qr_and_attachment_hashes(
        ...
        lambda _: [
            {
                ...
                "qr_type": "QRCODE",
                "qr_data": "https://qr.test/login",
                "url": "https://qr.test/login",
                "domain": "qr.test",
                "risk_score": 15,
            }
        ],
    ...
    for name, value in {
        ...
        "Reply-To": "support@reply.test",
        "Return-Path": "bounce@return.test",
        "Message-ID": "<id@message.test>",
...
def test_all_available_hash_algorithms_are_exported_without_lookup():
    records = collect_observables(
        urls=[],
        attachments=[
            {"filename": "a.bin", "md5": "a" * 32, "sha1": "b" * 40, "sha256": "c" * 64}
        ],
...
def test_no_silent_fifty_observable_cap():
    records = collect_observables(
        urls=extract_urls(" ".join(f"https://site-{n}.test/login" for n in range(50))),
        attachments=[{"filename": "a.bin", "sha256": "a" * 64}],
        url_intelligence=None,
    )
...
def test_legacy_external_evidence_has_no_score_verdict_or_coverage_effect():
    ...
    enriched = calculate_risk(
        auth,
        [{"malicious": 50}],
        [{"malicious": 50}],
        [{"pulse_count": 50}],
        ip_reputation=[{"risk_score": 90}],
        passive_dns=[{"risk_score": 90}],
        domain_intelligence={
            "whois_results": [{"age_days": 1, "risk_score": 90}],
            "dns_results": [{"risk_score": 90}],
        },
        landing_pages=[{"risk_score": 90, "password_fields": 3}],
    )
    ...
    bundle = build_evidence_bundle(
        ...
        vt_url_reports=[{"state": "malicious", "malicious": 50}],
    )
    assert not any(item["category"] == "threat_intel" for item in bundle["evidence"])


def test_ai_and_repeated_credential_keywords_cannot_decide_phishing(
    no_network, tmp_path
):
    body = " ".join(f"https://service.test/login/verify/account/{n}" for n in range(40))
    result = PhishingPipeline(
        upload_dir=str(tmp_path), events_jsonl_path=""
    ).analyze_raw("Subject: Reference links\n\n" + body)
...
def test_jsonl_write_failure_keeps_analysis_and_event_for_retry(no_network, tmp_path):
    ...
    result = PhishingPipeline(
        ...
        events_jsonl_path=str(blocked / "events.jsonl"),
    ).analyze_file(str(ROOT / "samples/phishing-en.eml"))
...
def test_jsonl_parallel_threads_and_processes_do_not_interleave(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=8) as pool:
        list(
            pool.map(
                lambda i: append_event(
                    {"id": f"thread-{i}", "text": "x\ny" * 1000}, path
                ),
                range(20),
            )
        )
    ...
    children = [
        subprocess.Popen(
            [sys.executable, "-c", program, str(path), str(i)],
            cwd=ROOT,
            env=os.environ.copy(),
        )
        for i in range(4)
